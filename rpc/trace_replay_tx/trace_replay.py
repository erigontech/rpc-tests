...
def make_request(json_rpc_cmd: str, target: str):
    ...
    try:
        ...
        response = requests.post(target, data=json_rpc_cmd, headers=headers)
...
def compare_trace_replay_transaction(block, tx_index, tx_hash: str):
    ...
    request = create_trace_transaction(tx_hash)
...
def main(argv) -> int:
    ...
    """
    opts, _ = getopt.getopt(argv[1:], "s:chn:", ['help', 'start=', 'continue', 'number='])
    continue_test = False
    start_block = 0
    start_tx = 0
    failed_request = 0
    max_failed_request = 0
    ...
    for block in range(int(start_block), 18000000):
        ...
        for txn in range(int(start_tx), len(transactions)):
            ...
            res = compare_trace_replay_transaction(block, txn, tx_hash)
            if res == 1:
                print ("Diff on block: ", block, " tx-index: ", txn, " Hash: ", tx_hash)
                if continue_test == 0:
                    return 1
                if max_failed_request != 0:
                    failed_request += 1
                    if failed_request >= max_failed_request:
                        return 1
