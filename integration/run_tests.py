...
def run_test(json_file: str, test_number, transport_type, config):
    ...
    for json_rpc in jsonrpc_commands:
        ...
        return result_code, error_msg, error_details, outcome
...
def main(argv) -> int:
        ...
        for test_rep in range(0, config.loop_number):  # makes tests more times
            ...
            for transport_type in tokenize_transport_type:
                                ...
                                if config.start_test == "" or test_number_in_any_loop >= int(config.start_test):
                                    if config.display_only_fail is False and config.req_test_number != "":
                                        ...
                                        if config.verbose_level == 1:
                                            test_results.append({
                                                "test_number": test_number_in_any_loop,
                                                "transport_type": transport_type,
                                                "test_name": json_test_full_name,
                                                "result": "SKIPPED",
                                                "error_message": ""
                                            })
                ...
                for test in tests_descr_list:
                    ...
                    result, error_msg, error_details, test_outcome = curr_future.result()
    ...
    if config.verbose_level == 1:
        report_filename = config.output_dir + "test_report.json"
        generate_json_report(
            report_filename, start_time, elapsed, global_test_number, available_tested_apis,
            config.loop_number, executed_tests, tests_not_executed, success_tests, failed_tests, test_results)
